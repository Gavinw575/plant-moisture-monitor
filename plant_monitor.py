...
# Setup logging
logging.basicConfig(
    filename='/home/chicken/plant_monitor.log', level=logging.DEBUG)
    ...
    def __init__(self, root, num_plants=40):
        try:
            ...
            self.monitor_thread = threading.Thread(
                target=self.monitor_moisture, daemon=True)
                    ...
                    try:
                        ...
                        for i in range(self.num_plants):
                            ...
                            if key in sensor_data:
                                self.channels[i] = type('obj', (), {'value': int(
                                    sensor_data[key] * 1023 / 3.3), 'voltage': sensor_data[key]})
                        # Broadcast data to all connected clients
                        self.broadcast_data(sensor_data)
                    except json.JSONDecodeError as e:
                        logging.error(
                            f"JSON decode failed from {addr}: {e} | Raw data: {data}")
            except socket.timeout:
...
def broadcast_data(self, sensor_data):
    ...
    def load_config(self):
        ...
        try:
            logging.debug(
                f"Loading config from {self.config_file} with num_plants={self.num_plants}")
            if not os.access(self.config_file, os.R_OK):
                logging.warning(
                    f"Config file {self.config_file} not readable, using default")
            ...
            if os.path.exists(self.config_file):
                ...
                for i in range(self.num_plants):
                    ...
                    if plant_key not in self.config:
                        logging.info(
                            f"Missing config for {plant_key}, using default")
    ...
    def setup_gui(self):
        ...
        tk.Label(title_frame, text="Plant Moisture Monitor", font=(
            'Arial', 20, 'bold'), fg='white', bg='#2E8B57').pack()
        ...
        scrollbar = ttk.Scrollbar(
            main_frame, orient="vertical", command=canvas.yview, style="TScrollbar")
        ...
        tk.Label(dry_frame, text="Dry Plants", font=(
            'Arial', 14, 'bold'), fg='white', bg='#2E8B57').pack(pady=5)
        self.dry_listbox = tk.Listbox(dry_frame, font=(
            'Arial', 12), width=15, height=15, bg='white')
        ...
        for i in range(self.num_plants):
            ...
            plant_frame = tk.Frame(
                scrollable_frame, bg='white', relief='raised', bd=2, width=190, height=300)
            plant_frame.grid(row=row, column=col, padx=5,
                             pady=5, sticky='nsew')
            plant_frame.grid_propagate(False)
            self.setup_plant_tile(plant_frame, i)
        canvas.configure(scrollregion=(
            0, 0, 600, 350 * (self.num_plants // 3 + 1)))
    ...
    def setup_plant_tile(self, parent, plant_id):
        ...
        plant_widgets['name_var'] = tk.StringVar(
            value=self.config[f'plant_{plant_id}']['name'])
        name_entry = tk.Entry(
            name_frame, textvariable=plant_widgets['name_var'], font=('Arial', 12), width=12)
        name_entry.pack(side='left', padx=5)
        name_entry.bind(
            '<FocusIn>', lambda e: name_entry.select_range(0, tk.END))
        name_entry.bind('<FocusOut>', lambda e,
                        pid=plant_id: self.update_plant_name(pid))

        plant_widgets['alert_label'] = tk.Label(name_frame, text="", font=(
            'Arial', 12, 'bold'), fg='red', bg='white', width=2)
        ...
        controls_frame = tk.Frame(
            main_frame, bg='white', width=120, height=180)
        ...
        if image_path and os.path.exists(image_path):
            try:
                ...
                plant_widgets['image_label'] = tk.Label(
                    controls_frame, image=plant_widgets['image'], bg='white')
            except Exception as e:
        ...
        plant_widgets['status_label'] = tk.Label(controls_frame, text="CHECKING...", font=(
            'Arial', 10, 'bold'), bg='white', fg='orange', width=14, height=2)
        plant_widgets['status_label'].pack(pady=5)

        plant_widgets['voltage_label'] = tk.Label(
            controls_frame, text="Voltage: --", font=('Arial', 8), bg='white', width=14, height=2)
        plant_widgets['voltage_label'].pack(pady=5)

        plant_widgets['moisture_progress'] = ttk.Progressbar(
            controls_frame, length=120, mode='determinate')
        plant_widgets['moisture_progress'].pack(pady=5)

        button_row_frame = tk.Frame(
            main_frame, bg='white', width=180, height=30)
    ...
    def select_image(self, plant_id):
        try:
            ...
            if plant_id >= len(self.plant_widgets):
                logging.error(
                    f"Invalid plant_id: {plant_id}, max: {len(self.plant_widgets)-1}")
                return
            ...
            if path:
                ...
                self.plant_widgets[plant_id]['image_label'].config(
                    image=self.plant_widgets[plant_id]['image'])
    ...
    def show_plant_details(self, plant_id):
        try:
            ...
            tk.Label(details_window, text=self.config[f'plant_{plant_id}']['name'], font=(
                'Arial', 16, 'bold'), bg='white').pack(pady=10)
            tk.Label(details_window, text=f"Status: {self.plant_widgets[plant_id]['status_label']['text']}", font=(
                'Arial', 12), bg='white').pack()
            tk.Label(details_window, text=f"Voltage: {self.plant_widgets[plant_id]['voltage_label']['text']}", font=(
                'Arial', 12), bg='white').pack()
            tk.Label(details_window, text=f"Dry Threshold: {self.config[f'plant_{plant_id}']['dry_threshold']:.2f} V", font=(
                'Arial', 12), bg='white').pack()
            tk.Label(details_window, text=f"Wet Threshold: {self.config[f'plant_{plant_id}']['wet_threshold']:.2f} V", font=(
                'Arial', 12), bg='white').pack()
            image_path = self.config[f'plant_{plant_id}']['image_path']
            if image_path and os.path.exists(image_path):
                try:
                    ...
                    tk.Label(details_window, image=photo,
                             bg='white').pack(pady=5)
                    details_window.image = photo
                except Exception as e:
                    logging.error(
                        f"Details image load failed for plant_{plant_id}: {e}")
            ...
            logging.error(
                f"Show plant details failed for plant_{plant_id}: {e}")
    ...
    def update_plant_name(self, plant_id):
        try:
            ...
            logging.error(
                f"Update plant name failed for plant_{plant_id}: {e}")
    ...
    def manual_thresholds(self, plant_id):
        try:
            ...
            manual_window.title(
                f"Set Thresholds {self.config[f'plant_{plant_id}']['name']}")
            ...
            plant_widgets['dry_threshold_var'] = tk.DoubleVar(
                value=self.config[f'plant_{plant_id}']['dry_threshold'])
            plant_widgets['wet_threshold_var'] = tk.DoubleVar(
                value=self.config[f'plant_{plant_id}']['wet_threshold'])

            tk.Label(manual_window, text="Dry Threshold (V):",
                     font=('Arial', 12), bg='white').pack(pady=5)
            dry_entry = tk.Entry(
                manual_window, textvariable=plant_widgets['dry_threshold_var'], font=('Arial', 12), width=10)
            dry_entry.pack(pady=5)
            dry_entry.bind(
                '<FocusIn>', lambda e: dry_entry.select_range(0, tk.END))

            tk.Label(manual_window, text="Wet Threshold (V):",
                     font=('Arial', 12), bg='white').pack(pady=5)
            wet_entry = tk.Entry(
                manual_window, textvariable=plant_widgets['wet_threshold_var'], font=('Arial', 12), width=10)
            wet_entry.pack(pady=5)
            wet_entry.bind(
                '<FocusIn>', lambda e: wet_entry.select_range(0, tk.END))
            ...
            def save_manual():
                try:
                    ...
                    if 0.0 <= dry <= 3.3 and 0.0 <= wet <= 3.3 and dry < wet:
                        ...
                        self.save_config()
                        logging.info(
                            f"Updated thresholds for plant_{plant_id}: dry={dry}, wet={wet}")
                        manual_window.destroy()
                    else:
                        tk.Label(
                            manual_window, text="Invalid values (0.0-3.3, dry < wet)", fg='red', bg='white').pack()
                except Exception as e:
                    tk.Label(manual_window, text="Enter valid numbers",
                             fg='red', bg='white').pack()
                    logging.error(
                        f"Threshold save failed for plant_{plant_id}: {e}")

            tk.Button(manual_window, text="Save", command=save_manual,
                      bg='green', fg='white', font=('Arial', 12, 'bold'), width=10, height=2).pack(pady=15)
        except Exception as e:
            logging.error(
                f"Manual thresholds failed for plant_{plant_id}: {e}")
    ...
    def get_moisture_status(self, voltage, plant_id):
        ...
        if voltage < dry_threshold:
            ...
            status_color = "#FF9999"
            progress_value = (voltage / dry_threshold) * \
                20 if dry_threshold > 0 else 0
            show_alert = True
        elif voltage > wet_threshold:
            status_text = "TOO WET"
            status_color = "#99CCFF"
            progress_value = 80 + ((voltage - wet_threshold) / (
                max_voltage - wet_threshold)) * 20 if max_voltage > wet_threshold else 100
            show_alert = False
        else:
            status_text = "PERFECT"
            status_color = "#99FF99"
            progress_value = 20 + ((voltage - dry_threshold) / (
                wet_threshold - dry_threshold)) * 60 if wet_threshold > dry_threshold else 20
            show_alert = False
            ...
            try:
                ...
                if not last_dry_check:
                    ...
                    try:
                        ...
                        logging.error(
                            f"Invalid last_dry_check format: {last_dry_check}")
                        do_daily_check = True
                ...
                for i in range(self.num_plants):
                    ...
                    status_text, status_color, progress_value, show_alert = self.get_moisture_status(
                        voltage, i)
                    ...
                    if show_alert:
                        if plant_name not in current_dry_plants:
                            self.root.after(
                                0, lambda name=plant_name: self.dry_listbox.insert(tk.END, name))
                            current_dry_plants.add(plant_name)
                    elif plant_name in current_dry_plants:
                        self.root.after(
                            0, lambda name=plant_name: self.remove_from_dry_list(name))
    ...
    def update_gui(self, plant_id, raw_value, voltage, status_text, status_color, progress_value, show_alert):
        try:
            ...
            if plant_id >= len(self.plant_widgets):
                logging.error(
                    f"Invalid plant_id in update_gui: {plant_id}, max: {len(self.plant_widgets)-1}")
                return
            ...
            for key in required_keys:
                if key not in widgets:
                    logging.error(
                        f"Missing widget key '{key}' for plant_{plant_id}")
                    return
        ...
        try:
            ...
            for widgets in self.plant_widgets:
                ...
                if not all(key in widgets for key in required_keys):
                    logging.error(
                        f"Missing widget keys in update_gui_error: {list(widgets.keys())}")
                    continue
                ...
                widgets['voltage_label'].config(
                    text="Voltage: ERROR", bg='white')
                widgets['status_label'].config(
                    text="SENSOR ERROR", fg="red", bg='white')
